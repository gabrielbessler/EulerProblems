...
#We only need to check up to root(maxVal), because if there is a factor over root(maxVal), there must also be one under root(maxVal)
...
for i in range(2, floor(maxVal**.5) + 1):
    ...
    if L != 0:
        ...
        #L[i**2 + i] = 0
        #L[i**2 + 2i] = 0...
        #While i**2 + ni < maxVal, so i(i + n) < maxVal, so (i + n) < maxVal / i, and n < (maxVal/i) - i
